...
char menu()
{
    ...
    printf("5. Particionar listas por edad.\n");
    printf("\n6. Salir\n\n");
    c=getch();
    return c;
}
...
/** \brief Genera una lista con personas > 18.
 *
 * \param Lista donde se encuentra la estructura.
 * \return  NULL. || Nueva lista, si se creo satisfactoriamente.
 *
 */
ArrayList* txtMayores(ArrayList* this)
{
    int i;
    ArrayList* listaMayores = al_newArrayList();
    Persona* p;
 ...
 *
 */
ArrayList* txtMenores(ArrayList* this)
{
    int i;
    ArrayList* listaMenores = al_newArrayList();
    Persona* p;
 ...
 *
 */
int guardarParticion(ArrayList* listaMayores, ArrayList* listaMenores)
{
    int i, retorno=-1, j;
    FILE *fMayores;
    FILE *fMenores;
    ...
    fMayores=fopen("pMayores.txt", "w+");
    fMenores=fopen("pMenores.txt", "w+");

    if((fMayores==NULL) || (fMenores==NULL))
    {
        retorno = -1;
    }
    else
    {
        rewind(fMayores);
        rewind(fMenores);
        for(i=0; i<al_len(listaMayores); i++)
        {
            p=newPersona();
            p = (Persona*)al_get(listaMayores,i);
            fflush(stdin);
            fprintf(fMayores,"%s,%s,%d\n",p->nombre,p->apellido, p->edad);
        }

        for(j=0; j<al_len(listaMenores); j++)
        {
            p=newPersona();
            p = (Persona*)al_get(listaMenores,j);
            fflush(stdin);
            fprintf(fMenores,"%s,%s,%d\n",p->nombre,p->apellido, p->edad);
        }
        retorno = 0;
    }
