...
int main()
{
    ...
    ArrayList* ListadoMayores = al_newArrayList();
    ArrayList* ListadoMenores = al_newArrayList();
    ...
    do
    {
        ...
        case '4':
            ...
            break;
        case '5':
            if(ListadoMayores!=NULL && ListadoMenores!=NULL)
            {
                ListadoMayores = txtMayores(ListadoPersonas);
                ListadoMenores = txtMenores(ListadoPersonas);

                pause("Particionado exitoso. (ENTER)");

                if((guardarParticion(ListadoMayores, ListadoMenores))==0)
                {
                    pause("Se crearon los archivos correctamente :)(ENTER)");
                }
                else
                {
                    pause("No se han podido crear los archivos :( (ENTER)");
                }
            }
            else
            {
                pause("No se realizo el particionado.");
            }
            break;
        case '6':
            printf("\nGuardar cambios S/N ?: ");
